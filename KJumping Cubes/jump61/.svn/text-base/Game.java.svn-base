...
class Game extends Observable {
    ...
    private void checkForWin() {
        if (gameInProgress()) {
            if (_board.getWinner() == RED || _board.getWinner() == BLUE) {
                announceWinner();
                _playing = false;
            }
        }
    ...
    private void setSpots(int r, int c, int spots, String color) {
        ...
        if (spots <= _board.neighbors(r, c)) {
            ...
            throw new IllegalArgumentException("spots must be smaller");
        }
    }
    ...
    private void setSize(int n) {
        if (n >= 2 && n <= 10) {
            ...
            throw new GameException("invalid size entered,try again!");
        }
    }
