...
public class PathsTesting {
    ...
    public void testingPath() {
        ...
        vdo.estimateSetter(7, 809.0);
        vdo.estimateSetter(1, 9999.99);
        vdo.estimateSetter(6, 40000.0);
        vdo.estimateSetter(8, 12000.00);
        vdo.setPaths();
        vdo.setWeight(4, 0.0);
        vdo.setWeight(2, 12.2);
        vdo.setWeight(5, 11.2);
        vdo.setWeight(6, 411111.2);
        vdo.setWeight(3, 18.7);
        assertEquals(9.1, vdo.getWeight(5, 3), 0.0);
        assertEquals(0.6, vdo.getWeight(5, 6), 0.0);
        assertEquals(4, vdo.getPredecessor(2), 0.1);
        assertEquals(0, vdo.getPredecessor(8));
        assertEquals(0, vdo.getPredecessor(7));
        List<Integer> order = vdo.pathTo(3);
        ArrayList<Integer> exp = new ArrayList<Integer>();
        exp.add(4);
        exp.add(2);
        exp.add(3);
        assertTrue(exp.equals(order));
