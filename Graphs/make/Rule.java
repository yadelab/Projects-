...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!commands.isEmpty() && !_commands.isEmpty()) {
            throw new IllegalStateException("non-empty command set");
        }
    ...
    private void checkFinishedDependencies() {
        for (int v : _depends.successors(getVertex())) {
            if (_depends.getLabel(v).isUnfinished()) {
                error("Dependecy is not built");
            }
        }
    }
    ...
    private boolean outOfDate() {
        ...
        for (int t : _depends.successors(getVertex())) {
            if (_depends.getLabel(t).getTime() > _time) {
                return true;
            }
        }
        return false;
