...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        Road rhod1 = _map.getLabel(segment.get(0), segment.get(1));
        for (int n = 1; n <= segment.size() - 2; n++) {
            Road rhod2 = _map.getLabel(segment.get(n), segment.get(n + 1));
            Direction dRection1 = rhod1.direction();
            Direction dRection2 = rhod2.direction();
            String rhodName1 = rhod1.toString();
            String name = dRection1.fullName();
            if (rhod1.toString().equals(rhod2.toString())
                && dRection1.equals(dRection2)) {
                Double countLength = rhod1.length() + rhod2.length();
                rhod1 = new Road(rhodName1, dRection1, countLength);
            } else {
                System.out.printf(seq + ". Take" + " " + rhod1.toString() + " "
                    + rhod1.direction().fullName() + " for"
                        + " %.1f" + " miles.\n", rhod1.length());
                rhod1 = rhod2;
                seq++;
            }
        }
        String vame = _map.getLabel(segment.get(segment.size() - 1)).toString();
        Road veryLastR = _map.getLabel(segment.get(segment.size() - 2),
                segment.get(segment.size() - 1));
        Direction lastD = veryLastR.direction();
        if (!rhod1.toString().equals(veryLastR.toString())
            && !lastD.equals(rhod1.direction())) {
            rhod1 = veryLastR;
        }
        System.out.printf(seq + ". Take" + " " + rhod1.toString()
                          + " " + rhod1.direction().fullName()
                          + " for" + " %.1f"
                          + " miles" + " " + "to" + " " + vame
                          + ".", rhod1.length());
        System.out.println();
    ...
    private void addRoad(String from, String name, double length,
        ...
        Road r = new Road(name, dir, length);
        Road rev = new Road(name, dir.reverse(), length);
        _map.add(v0, v1, r);
        _map.add(v1, v0, rev);
    }
    ...
    private static class RoadMap extends LabeledGraph<Location, Road> {
        /** An empty RoadMap. Had replace with solution comment.*/
        RoadMap() {
