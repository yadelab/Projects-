...
class CommandInterpreter {
    ...
    void loadStatement() {
        _input.next("load");
        String m = name();
        Table br = Table.readTable(m);
        _database.put(m, br);
        _input.next(";");
        System.out.println("Loaded " + m + ".db");
    }
    ...
    void printStatement() {
        _input.next("print");
        String r = _input.peek();
        Table t = tableName();
        _input.next(";");

        System.out.println("Contents of " + r + ":");
        if (t != null) {
            t.print();
        } else {
            throw error("a null table", r);
        }
    }
    ...
    Table selectClause() {
        ...
        String d = _input.peek();
        if (d.equals("where")) {
            if (potato == false) {
                mector = conditionClause(table1);
            } else {
                mector = conditionClause(table1, table2);
            }
        } else {
            mector = new ArrayList<Condition>();
        }
        if (potato) {
            outPut = table1.select(table2, vector, mector);
        } else {
            outPut = table1.select(vector, mector);
        }
        return outPut;
    }
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        ArrayList<Condition> emptyyy = new ArrayList<Condition>();
        _input.next("where");
        emptyyy.add(condition(tables));
    ...
    Condition condition(Table... tables) {
        ...
        Column n = new Column(cl, tables);
        String rln = _input.next(Tokenizer.RELATION);
        String eqls = _input.peek();
        Condition cc;
        if (eqls.startsWith("'")) {
            String str = literal();
            cc = new Condition(n, rln, str);

        } else {
            String str = columnName();
            Column notStr = new Column(str, tables);
            cc = new Condition(n, rln, notStr);
        }
        return cc;
    }
